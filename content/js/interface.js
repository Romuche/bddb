const button = function (...classNames) {
...
function buildButtons(url) {
    if (isProfilePage(url)) {
        // Build photo export button
        waitForElement(".react-aspect-ratio-placeholder").then(() => {
            document.querySelector(".react-aspect-ratio-placeholder").appendChild(button("bddb-button", "photo-button"));
        });
    }

    if (isChatPage(url)) {
        // Build chat export button
        waitForElement(".chat").then(() => {
            document.querySelector(".chat").appendChild(button("bddb-button", "chat-button"));
        });
    }
}
